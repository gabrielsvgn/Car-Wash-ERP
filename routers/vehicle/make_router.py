...
@make_router.get("/list", response_model=list[MakeResponseSchema])
...
@make_router.get("/list/{idcar_make}", response_model=MakeResponseSchema)
...
@make_router.put("/update/{idcar_make}")
...
@make_router.delete("/delete/{idcar_make}")
async def delete_car_make(idcar_make: int, user: User = Depends(verify_token), session: Session = Depends(get_session)):
     car_make = session.query(Car_make).filter(Car_make.idcar_make==idcar_make).first()
     if not car_make:
          raise HTTPException(status_code=404, detail="Car make was not found")
     session.delete(car_make)
     session.commit()
     return {"message": "The make was successfully deleted"}